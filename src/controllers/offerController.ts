  ...
  static async getPreview(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const previewHtml = `
        ...
        <html>
          <head>
            <title>${offer.name} - Preview</title>
            <style>
              body { font-family: system-ui, sans-serif; max-width: 800px; margin: 40px auto; padding: 20px; background: #0a0a0f; color: #e4e4e7; }
              .card { background: #18181b; border: 1px solid #27272a; border-radius: 12px; padding: 24px; }
              .badge { background: #059669; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px; }
              .field { margin: 16px 0; padding: 12px; background: #1f1f23; border-radius: 8px; }
              .label { color: #a1a1aa; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px; }
              .value { color: #e4e4e7; font-size: 16px; margin-top: 4px; }
            </style>
          </head>
          <body>
            <div class="card">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h1 style="font-size: 24px; font-weight: 600;">${offer.name}</h1>
                <span class="badge">${offer.status}</span>
              </div>
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                <div class="field">
                  <div class="label">Network</div>
                  <div class="value">${offer.network?.name || 'N/A'}</div>
                </div>
                <div class="field">
                  <div class="label">Category</div>
                  <div class="value">${offer.category || 'N/A'}</div>
                </div>
                <div class="field">
                  <div class="label">Country</div>
                  <div class="value">${offer.country || 'All'}</div>
                </div>
                <div class="field">
                  <div class="label">Payout</div>
                  <div class="value">$${offer.payout?.toFixed(2) || 'N/A'}</div>
                </div>
                <div class="field">
                  <div class="label">Daily Cap</div>
                  <div class="value">${offer.dailyCap || 'Unlimited'}</div>
                </div>
                <div class="field">
                  <div class="label">Hourly Cap</div>
                  <div class="value">${offer.hourlyCap || 'Unlimited'}</div>
                </div>
                ${offer.startDate ? `
                <div class="field">
                  <div class="label">Start Date</div>
                  <div class="value">${new Date(offer.startDate).toLocaleDateString()}</div>
                </div>
                ` : ''}
                ${offer.endDate ? `
                <div class="field">
                  <div class="label">End Date</div>
                  <div class="value">${new Date(offer.endDate).toLocaleDateString()}</div>
                </div>
                ` : ''}
                ${offer.deviceTargeting ? `
                <div class="field" style="grid-column: 1 / -1;">
                  <div class="label">Device Targeting</div>
                  <div class="value">${offer.deviceTargeting}</div>
                </div>
                ` : ''}
                ${offer.browserTargeting ? `
                <div class="field" style="grid-column: 1 / -1;">
                  <div class="label">Browser Targeting</div>
                  <div class="value">${offer.browserTargeting}</div>
                </div>
                ` : ''}
              </div>
            </div>
