...
export class AIService {
  static async getScores() {
    ...
    const [trafficQuality, fraudScore] = await Promise.all([
      this.calculateTrafficQuality(),
      this.calculateFraudScore(),
    ]);

    const optimizationLevel = this.calculateOptimizationLevel(trafficQuality, fraudScore);
  ...
  static async getOffers(type: 'best' | 'worst') {
    ...
    if (type === 'best') {
      offers = await this.getBestOffers();
    } else if (type === 'worst') {
      offers = await this.getWorstOffers();
    }
  ...
  static async getForecast(metric: string) {
    ...
    const forecast = this.forecastData(historical);
  ...
  static async runOptimization() {
    // Run AI optimization in background
  ...
  private static async calculateTrafficQuality(): Promise<number> {
    const stats = await prisma.$queryRaw`
      SELECT 
        ...
        AVG(CASE WHEN conv.status = 'APPROVED' THEN 1 ELSE 0 END) as approval_rate
      FROM "clicks" c
    ...
    let score = 0;
    const conversionRate = data.total_clicks > 0 ? data.total_conversions / data.total_clicks : 0;
    score += conversionRate * 40;
    score += Number(data.approval_rate) * 30;
    const volumeScore = Math.min(data.total_clicks / 1000, 1) * 30;
    score += volumeScore;
  ...
  private static async getHistoricalData(metric: string): Promise<any[]> {
  ...
  private static async generateRecommendations(): Promise<any[]> {
    ...
    const lowConvertingOffers = await this.getWorstOffers();
    ...
    const bestOffers = await this.getBestOffers();
  ...
  private static async optimizeTraffic(): Promise<void> {
    logger.info('AI optimization started');
    
    // Analyze data and make optimization decisions
    // 1. Adjust routing weights
    // 2. Pause underperforming offers
    // 3. Allocate more traffic to high-performing offers
    // 4. Update fraud thresholds
    // 5. Adjust bid prices
